...
def to_2tuple(x):
    return (x, x) if isinstance(x, tuple) else (x, x)
...
class RoPE(nn.Module):
    ...
    def __init__(self, shape, base=10000):
        ...
        # angles
        theta_ks = 1 / (base ** (jt.arange(k_max) / k_max))
        angles = jt.cat([t.unsqueeze(-1) * theta_ks for t in jt.meshgrid([jt.arange(d) for d in channel_dims])], dim=-1)

        # rotation
        rotations_re = jt.cos(angles).unsqueeze(dim=-1)
        rotations_im = jt.sin(angles).unsqueeze(dim=-1)
        rotations = jt.cat([rotations_re, rotations_im], dim=-1)
    ...
    def execute(self, x):
        ...
        x_re, x_im = x[..., :1], x[..., 1:]
        pe_x = jt.cat([x_re * rotations_re - x_im * rotations_im, x_im * rotations_re + x_re * rotations_im], dim=-1)
...
def mmla_base(**kwargs):
    model = MLLA(depths=[3, 6, 21, 6], num_heads=[3, 6, 12, 24], **kwargs)
    return model
